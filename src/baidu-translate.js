...
function md5(string) {
  ...
  var result = '';
  for (var i = 0; i < 4; i++) {
    result += hex_chr((state[i] >> 0) & 0xFF) +
              hex_chr((state[i] >> 8) & 0xFF) +
              hex_chr((state[i] >> 16) & 0xFF) +
              hex_chr((state[i] >> 24) & 0xFF);
  }
  return result;
}
...
export async function translateBaidu(text, appId, secretKey) {
  ...
  if (data.trans_result && data.trans_result.length > 0) {
    let translated = data.trans_result.map(item => item.dst).join('\n');
    if (truncated) {
      translated += '\n\n[原文超过1800字符，已截断翻译]';
    }
